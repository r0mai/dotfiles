...
override_git_prompt_colors() {
  ...
  function sugoi_target_ps1() {
    if [ ! -e .sugoi.target ]; then
      return
    fi

    echo "[$(sugoi target 2>&1 | cut -d' ' -f 2)]"
  }
  ...
  GIT_PROMPT_END_USER=" ${BoldGreen}\$(sugoi_target_ps1)${ResetColor} \n${White}${Time12a}${ResetColor} $ "
  GIT_PROMPT_END_ROOT=" \$(sugoi_target_ps1) \n${White}${Time12a}${ResetColor} # "
